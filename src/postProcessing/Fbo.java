...
public class Fbo {
	...
	private int multisampleAmount = 4;
	...
	public Fbo(int width, int height, int depthBufferType) {
		this.width = width;
		this.height = height;
		initialiseFrameBuffer(depthBufferType, 1);
	}
	
	/**
	 * Creates an FBO of a specified width and height using multisampling.
	 * 
	 * @param width
	 *            - the width of the FBO.
	 * @param height
	 *            - the height of the FBO.
	 */
	public Fbo(int width, int height, float buffers) {
		this.width = width;
		this.height = height;
		this.multisample = true;
		this.buffers = (int) buffers;
		if(buffers > 16) {
			System.exit(-1);
		}
		initialiseFrameBuffer(DEPTH_RENDER_BUFFER, this.buffers);
	}
	...
	public void cleanUp() {
		...
		for(int i = 0; i < colorBuffers.length; i++) {
			GL30.glDeleteRenderbuffers(colorBuffers[i]);
		}
	}
	 ...
	 *            FBO.
	 */
	private void initialiseFrameBuffer(int type, int buffers) {
		colorBuffers = new int[buffers];
		createFrameBuffer();
		if(multisample) {
			for(int i = 0; i < colorBuffers.length; i++) {
				colorBuffers[i] = createMultisampleColorAttachment(GL30.GL_COLOR_ATTACHMENT0 + i);
			}
		} else {
	...
	private void determineDrawBuffers() {
		IntBuffer drawBuffers = BufferUtils.createIntBuffer(colorBuffers.length);
		for(int i = 0; i < colorBuffers.length; i++) {
	...
	private int createMultisampleColorAttachment(int attachment) {
		...
		GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, multisampleAmount, GL11.GL_RGBA8, width, height);
	...
	private void createDepthBufferAttachment() {
		...
		if(!multisample) {
			...
			GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, multisampleAmount, GL14.GL_DEPTH_COMPONENT24, width, height);
		}
